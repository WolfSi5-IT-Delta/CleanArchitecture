...
export const OneLineCellAnswer = ({value, row}) => {
  return (
    <div className="w-full text-left overflow-hidden whitespace-pre-line">
      <Link href={route("admin.teacher.lesson", row.original.id)}>{value}</Link>
    </div>
  );
};

export const OneLineCellCurriculum = ({value, row}) => {
  console.log(row)
  return (
    <div className="w-full text-left overflow-hidden whitespace-pre-line">
      <Link href={route("admin.curriculum.edit", row.original.id)}>{value}</Link>
    </div>
  );
};
