...
export default function ({ user }) {
  ...
  const [avatarFormImg, setAvatarFormImg] = useState(data.avatar || '/img/no-user-photo.jpg');
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    if (passwordsMatch()) {
      post('/accept-invite' + location.search);
    }
  };
    ...
    <>
      <header>
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold leading-tight text-gray-900 text-center">New User Registration</h1>
        </div>
      </header>
      <main>
            ...
            {Object.values(errors).length ? (
              <div className="text-red-600">
                ...
                {Object.values(errors).map((e) => (
                  <p className="mt-2 text-sm text-red-600">{e}</p>
                ))}
              </div>
            ) : ''}
